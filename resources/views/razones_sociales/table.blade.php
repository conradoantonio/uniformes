        ...
        @foreach($items as $item)
            <tr>
                ...
                <td class="text-center align-middle">
                    <button class="btn btn-success btn-sm generar-excel-historial" data-row-name="{{$item->nombre}}" data-row-id="{{$item->id}}" data-toggle="tooltip" data-placement="top" title="Descargar excel de historial de uniformes"><i class="mdi mdi-file-excel"></i></button>
                    @if( auth()->user()->permisos()->where('permisos.alias', 'razones_editar')->exists() )
                        @if(! $item->deleted_at )
                        ...
                        @else
                            <button class="btn btn-info btn-sm enable-row" data-row-id="{{$item->id}}" data-change-to="1" data-toggle="tooltip" data-placement="top" title="Habilitar"><i class="mdi mdi-check"></i></button>
                        @endif
                    @endif
                </td>
                @endif
