    ...
    <tbody>
        @foreach($items as $item)
            <tr>
                ...
                <td class="align-middle">{{$item->clientes->count()}}</td>
                ...
                <td class="text-center align-middle">
                    <button class="btn btn-info btn-sm generar-excel-facturas" data-row-name="{{$item->nombre}}" data-row-id="{{$item->id}}" data-toggle="tooltip" data-placement="top" title="Descargar excel de facturas de clientes"><i class="mdi mdi-format-list-checkbox"></i></button>
                    @if( auth()->user()->permisos()->where('permisos.alias', 'razones_editar')->exists() )
                        @if(! $item->deleted_at )
