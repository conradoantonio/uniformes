...
class NotasDeCreditoController extends Controller
{
    ...
    public function index(Request $req)
    {
        ...
        if ( $req->ajax() ) {
            return view('notas_credito.table', compact('items'));
        }
        return view('notas_credito.index', compact('items', 'clientes', 'razones', 'menu', 'title'));
    }
    ...
    {
        $req->request->add([ 'user' => auth()->user(), 'tipo_recibo_id' => 1 ]);
    ...
    {
        $title = "Formulario de nota de crédito";
        $menu = "Notas de crédito";
        $item = null;
        ...
        return view('notas_credito.form', compact(['item', 'clientes', 'facturas', 'menu', 'title']));
    }
    ...
    public function save(Request $req)
    {
        ...
        return response(['msg' => 'Registro guardado correctamente', 'url' => url('notas-de-credito'), 'status' => 'success'], 200);
    }
    ...
    {
        $item = Recibo::where('id', $req->id)->where('tipo_recibo_id', 1)->first();
        ...
        return response(['msg' => 'Registro actualizado correctamente', 'url' => url('notas-de-credito'), 'status' => 'success'], 200);
    }
    ...
    public function delete(Request $req)
    {
        ...
        $items = Recibo::whereIn('id', $req->ids)->where('tipo_recibo_id', 1)
        ->delete();
        ...
        if ( $items ) {
            return response(['msg' => 'Éxito eliminando '.$msg, 'url' => url('notas-de-credito'), 'status' => 'success'], 200);
        } else {
            return response(['msg' => 'Error al cambiar el status de '.$msg, 'status' => 'error', 'url' => url('notas-de-credito')], 404);
        }
    }
    ...
    public function export(Request $req)
    {
        ...
        foreach ( $items as $item ) {
            ...
            $rows [] = [
                ...
                'Tipo de movimiento'   => 'Nota de crédito',
                'Factura'              => $num_factura,
        ...
        Excel::create('Listado de notas de crédito', function($excel) use ($rows, $total, $periodo) {
